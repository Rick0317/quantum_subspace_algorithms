...
output_filename = f'main_outputs/{molecule}_{bond_length}_VO_parallel'
...
def evaluate_off_diagonal(i, j, Hsub, factorized_tapered_statevectors, UCSF_information, configs, Hqub, Nqubits, output_filename):
    with open(output_filename, 'a') as f:
        print(f'{i, j}', file=f)
...
results = Parallel(n_jobs=-1)(delayed(evaluate_off_diagonal)(i, j, Hsub, factorized_tapered_statevectors, UCSF_information, configs, Hqub, Nqubits, output_filename) for i,j in ij_pairs)
